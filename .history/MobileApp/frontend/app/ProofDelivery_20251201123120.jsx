...
export default function ProofDelivery() {
  ...
  try {
    ...
    // IMPORTANT: doit s’appeler "file" comme dans @RequestParam("file")
    ...
    const res = await apiMobile.post('/mobile/preuve', formData, {
      headers: { 'Content-Type': 'multipart/form-data' },
    });

    console.log('Preuve envoyée', res.status);
    ...
    console.log('ERR preuve =>', error.response?.status, error.response?.data);
    Alert.alert('Erreur', "Impossible d'envoyer la preuve de livraison");
  } finally {
    setIsLoading(false);
  }
};
