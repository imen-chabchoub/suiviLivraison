...
export default function ProofDelivery() {
  ...
  try {
    ...
    // 1) On stocke localement pour l’UI
    setCurrentLocation({
      latitude: lat,
      longitude: lng,
      timestamp,
    ...
    // 2) On envoie vers l’API POST /mobile/livreur/position/{livreurId}
    // selon ce que ton contrôleur attend :
    // - soit sans body
    // - soit avec body JSON { latitude, longitude, timestamp }
    // Exemple le plus courant : body JSON
    await apiMobile.post(
      `/mobile/livreur/position/${userId}`,null,
      {params:{
        latitude: lat,
        longitude: lng,
        timestamp,
      },}
    ); // Axios sérialise en JSON par défaut [web:255][web:273]

    Alert.alert('Succès', 'Localisation enregistrée avec succès');
  } catch (error) {
    console.error('Erreur géolocalisation:', error?.response?.data || error);
    Alert.alert('Erreur', "Impossible d'obtenir ou d’enregistrer la localisation");
  }
};
  ...
  try {
    ...
    await apiMobile.put(
     ...
     console.log('Statut mis à jour', resStatut.status);
    // 3) Succès
    Alert.alert(
      'Livraison validée !',
