...
const [loading, setLoading] = useState(false);
  ...
  const fetchAssignedDeliveries = async () => {
    try {
      setLoading(true);
      const userId = await AsyncStorage.getItem('userId'); // ID du livreur connecté
      if (!userId) {
        console.log('Aucun userId trouvé en storage');
        return;
      }

      // Appel backend : GET /mobile/livraison/assignees/{idLivreur}
      const res = await apiMobile.get(`/mobile/livraison/assignees/${userId}`);
      const livraisons = res.data;

      // Adapter les champs pour ton DeliveryCard
      const mapped = livraisons.map(l => ({
        code: l.codeBarre || `LIV-${l.id}`,
        client: l.client ? `${l.client.prenom} ${l.client.nom}` : 'Client',
        address: l.adresse || (l.client ? l.client.adresse : 'Adresse inconnue'),
        phone: l.client ? l.client.telephone : '',
        status: l.statut || 'EN COURS',
        raw: l, // garder l’objet brut si tu en as besoin plus tard
      }));

      setDeliveries(mapped);
    } catch (err) {
      console.log('Erreur fetch livraisons assignées', err?.response?.data || err?.message);
    } finally {
      setLoading(false);
    }
  };
